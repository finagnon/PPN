...
static const char *VPREC_INST_MODE_STR[] = {"arguments", "operations", "all","none"};
...
//CONVERSION
struct fixed32 vprec_float_to_fixed32(float num, int8_t FRAC_TARGET) {
 ...
 //to binary64
 struct fixed64 vprec_float_to_fixed64(float num, int8_t FRAC_TARGET){
     
     structfixed64 f64 ={.num = (int64_t) (num * (1 << FRAC_TARGET)), .FRAC_PART = FRAC_TARGET, .INT_part = CHAR_WL-FRAC_TARGET)};
     
     if (f64.num > LONG_MAX_ENCODING) printf("OVERFLOW!!--CONVERSION\n");
  else if (f64.num < -1 * LONG_MIN_ENCODING) printf("UNDERFLOW!!--CONVERSION\n");
  printf("%f=%d (Q%d.%d)\n", num, f64.num, f64.INT_PART, f64.FRAC_PART);    
...
//32bits
struct fixed32 _interflop_add_float(struct fixed32 a, struct fixed32 b) {
  struct fixed64 acc;
...
struct fixed32 _interflop_sub_float(struct fixed32 a, struct fixed32 b) {
  struct fixed32 c = {.num = -b.num, .FRAC_PART=b.FRAC_PART, .INT_PART=b.INT_PART};
  
  return fixed32_add(a, c);
}

struct fixed32 _interflop_mul_float(struct fixed32 a, struct fixed32 b) {
  struct fixed64 acc;
...
struct fixed64 _interflop_add_double(struct fixed64 a, struct fixed64 b) {
  struct fixed64 acc;
  ...
  struct fixed64 result = {.num=acc.num, .FRAC_PART=acc.FRAC_PART, .INT_PART=acc.INT_PART};
  printf("%d+%d=%d (Q%d.%d)\n", a.num, b.num, result.num, result.INT_PART, result.FRAC_PART);
  
  return result;
}

struct fixed64 _interflop_sub_double(struct fixed64 a, struct fixed64 b) {
  struct fixed64 c = {.num = -b.num, .FRAC_PART=b.FRAC_PART, .INT_PART=b.INT_PART};
  
  return fixed64_add(a, c);
}

struct fixed64 _interflop_mul_double(struct fixed64 a, struct fixed64 b) {
  struct fixed64 acc;
  acc.num = ((__int128_t)a.num*(__int128_t)b.num);
  acc.num += (1<<(a.FRAC_PART-1));
  acc.FRAC_PART = a.FRAC_PART + b.FRAC_PART - CHAR_WL;
  acc.INT_PART  = a.INT_PART  + b.INT_PART;
  
  struct fixed64 result = {.num=acc.num>>CHAR_WL, .FRAC_PART=acc.FRAC_PART, .INT_PART=acc.INT_PART};
  printf("%d*%d=%d (Q%d.%d)\n", a.num, b.num, result.num, result.INT_PART, result.FRAC_PART);
